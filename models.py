...
class Attention(Module):
    def __init__(self, nembed):
        ...
        self.att_weight = Parameter(torch.FloatTensor(nembed, 1))
        self.reset_parameters()

    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.att_weight.size(0))
        self.att_weight.data.uniform_(-stdv, stdv)

    def forward(self, node_embed):
        att_scores = torch.matmul(node_embed, self.att_weight)
        att_scores = torch.tanh(att_scores)
        att_scores = torch.exp(att_scores)
        att_scores = att_scores / torch.sum(att_scores, dim=0)
        att_scores = att_scores.transpose(-1, -2)
        node_embed_att = torch.matmul(att_scores, node_embed)
        return node_embed_att
    # def forward(self, node_embed):
    #     att_scores = torch.matmul(node_embed, self.att_weight)
    #     att_scores = F.softmax(att_scores, dim=0)
    #     att_scores = att_scores.transpose(-1, -2)
    #     node_embed_att = torch.matmul(att_scores, node_embed)
    #     node_embed_att = node_embed + node_embed_att  # 跳跃连接
    #     return node_embed_att
...
class Decoder(Module):
    ...
    def __init__(self, nembed, dropout=0.1):
        super(Decoder, self).__init__()
        self.dropout = dropout
        self.attention = Attention(nembed)
        self.de_weight = Parameter(torch.FloatTensor(nembed, nembed))
        self.reset_parameters()
    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.de_weight.size(1))
    ...
    def forward(self, node_embed):
        node_embed_att = self.attention(node_embed)
        combine = F.linear(node_embed_att, self.de_weight)
        adj_out = torch.sigmoid(torch.mm(combine, combine.transpose(-1,-2)))
        return adj_out