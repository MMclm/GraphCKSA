...
def load_cite(path="data/citeseer/", dataset="citeseer"):  # modified from code: pygcn
    """Load citation network dataset (cora only for now)"""
    print('Loading {} dataset...'.format(dataset))
    # 从文件中读取特征矩阵、标签等数据
    idx_features_labels = np.genfromtxt("{}{}.txt".format(path, dataset),
                                        dtype=np.dtype(str))
    features = sp.csr_matrix(idx_features_labels[:, 1:-1], dtype=np.float32)
    labels = idx_features_labels[:, -1]
    # print('labels-------------', labels)
    ...
    # 构建标签类别字典
    classes_dict = {c: np.arange(len(set_labels))[i] for i, c in enumerate(set_labels)}
    classes_dict = {'Agents': 0, 'DB': 1, 'IR': 2, 'AI': 3, 'ML': 4, 'HCI': 5}  # 将标签替换成类别号
    # ipdb.set_trace()
    labels = np.array(list(map(classes_dict.get, labels)))
    # 构建图
    # build graph
...
def load_pub(path="data/pubmed/", dataset="pubmed"):
...
def load_graph(path="data/graph/", dataset="graph"):#modified from code: pygcn
    """Load citation network dataset (cora only for now)"""
    #input: idx_features_labels, adj
    #idx,labels are not required to be processed in advance
    #adj: save in the form of edges. idx1 idx2 
